...
with torch.no_grad():
    ...
    for i in range(num_time_steps):
        input = input.view(1, -1, 1)  # Add an extra dimension to match the input size
        output = model(input)
        prediction = output[0, -1, 0].item()  # Access the scalar value from the tensor
        predictions.append(prediction)
        input = torch.cat((input[:, 1:, :], output.unsqueeze(0)))
