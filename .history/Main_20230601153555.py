...
st.title('Resistance and Support Levels of a Stock')
...
resistance_level = max(high_prices[:-1])
support_level = min(low_prices[:-1])

# Display the resistance and support levels
...
# Plot the stock prices with resistance and support levels
plt.figure(figsize=(10, 6))
plt.plot(stock_data['Date'], close_prices, label='Close')
plt.axhline(resistance_level, color='r', linestyle='--', label='Resistance')
plt.axhline(support_level, color='g', linestyle='--', label='Support')
plt.xlabel('Date')
plt.ylabel('Price')
plt.title(f'Stock Price with Resistance and Support Levels ({stock_symbol})')
plt.legend()
